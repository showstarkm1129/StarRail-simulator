...
| パス | 役割 |
|---|---|
| `index.html` | エントリ HTML。タブ構成 (速度・行動値 / 戦闘シミュ / キャラビルド / 限界効用逓減 / 遺物 / AIアシスタント) |
...
| ファイル | 役割 |
|---|---|
| `buildUI.js` | キャラビルドタブの UI (キャラ/光円錐/軌跡 + 遺物倉庫からの装備 + 最終ステ表示 + ビルド保存)。ビルドを作る責務だけを持ち、比較は限界効用逓減タブへ渡す |
...
## ユーザーとの対話で求めること
- ユーザーはプロジェクトの全容を完璧には把握しきれていないことを前提に進める。
- ユーザーが今後の構想や新たな機能を思いついた際は今後活用するためにmemo.txtに記録する。
- やり取りを通じて新たに判明した教訓もmemo.txtに記録する。

なお、CodexとClaudeを同時に使用している。両方の環境で利用できるような形式で進めることを意識すること。
